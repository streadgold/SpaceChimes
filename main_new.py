...
def audio_playback_thread():
    ...
    while True:
        ...
        stream.write(sound_buffer[:1024].astype(np.float32).tostring())
...
def check_pass_times(debris_data):
    ...
    for debris in debris_data:
        pass_datetime = datetime.strptime(debris['culmination_time'], '%Y-%m-%d %H:%M:%S')
        # Calculate expiry time as 30 minutes after pass_datetime
        expiry_time = pass_datetime + timedelta(minutes=30)
         
        if pass_datetime > (now - timedelta(minutes=1)) and now >= pass_datetime and now <= expiry_time and debris['object_id'] not in triggered_alerts:
            ...
            print(f"{debris['object_id']} {debris['object_name']} is overhead {debris['rcs']} {debris['altitude_km']}!")

debris_data = load_debris_data()

while True:
    # this calls the function that loops through the pass time list
